...
public abstract class AbstractDefaulterBase<T>
{
    ...
    /// <returns>A <see cref="ForEachRuleBuilder{T, TProperty}"/> for further configuration.</returns>
    public ForEachRuleBuilder<T, TProperty> ForEach<TProperty>(
        Expression<Func<T, IEnumerable<TProperty>>> expression
    )
    {
        var rule = new Rule<T>((MemberExpression)expression.Body);
        _rules.Add(rule);
        return new ForEachRuleBuilder<T, TProperty>(rule);
    }
}
