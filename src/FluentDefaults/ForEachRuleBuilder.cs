...
/// </summary>
public sealed class ForEachRuleBuilder<T, TProperty>
{
    private readonly Rule<T> _rule;

    internal ForEachRuleBuilder(Rule<T> rule)
    {
        _rule = rule;
    }
    ...
    public ForEachDefaultForRuleBuilder<T, TProperty> DefaultFor<TElementProperty>(
        Expression<Func<TProperty, TElementProperty>> expression
    )
    {
        _rule.ChildMemberExpression = (MemberExpression)expression.Body;
        return new ForEachDefaultForRuleBuilder<T, TProperty>(_rule);
    }
}
